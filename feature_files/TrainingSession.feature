...
Feature: This feature is to create training session, add career, promotion and session
  ...
  Scenario: To verify whether the admin user is able to login to the application
    Given I launch the url "http://elearningm1.upskills.in/"
    And I enter the valid username "admin" in username textbox
    Then I enter the valid password "admin@123" in password textbox
  ...
  Scenario: To verify whether application allows admin to create training session
    ...
    And I enter valid data "ML training session" in Session name textbox
    Then I select valid coach name "abc" in coach name dropdown
    And I click on Next step button in Add a training session page
    Then I select course "MachineLearning (ML)" in course list window
    And I click on > button
    And I click on Next step button in Add courses to this session page
    And I enter student name "dinesh" in Portal users list
  ...
  Scenario: To verify whether application allows admin to add career, promotion & add a session in career and promotions link
    ...
    Then I enter valid data "QA" in Name textbox of Add career page
    ...
    And I click on Add icon
    Then I enter valid data "QA for Tester" in Name textbox of Add promotion page
    ...
    And I enter valid data "API training session" in Session name textbox
    Then I select valid coach name "abc" in coach name dropdown
    And I click on Next step button in Add a training session page
    Then I select course "APITetsting (API)" in course list window
    And I click on > button
    And I click on Next step button in Add courses to this session page
    And I enter student name "dinesh" in Portal users list
    Then I click on the student name link
